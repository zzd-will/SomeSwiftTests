...
class CSPageViewController: UIViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIScrollViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UIToolbarDelegate {
    // MARK: - Protocols
    weak var datasource: CSPageViewControllerDataSource?
    weak var delegate: CSPageViewControllerDelegate?
    ...
    var tabbarStyle: CSTabbarStyle {
        get {
            return .Blurred
        }
    }
    var tabbarPosition: CSTabbarPosition {
        get {
            return .Top
        }
    }
    ...
    private func tabViewAtIndex(index: Int) -> CSTabView? {
        if let tabViewContent: UIView = datasource?.tabViewForPageAtIndex(self, index: index) {
            var tabView: CSTabView
            
            switch tabbarPosition {
            case .Top, .Bottom:
                if let theWidth: CGFloat = delegate?.widthForTabAtIndex?(index) {
                    tabView = CSTabView(frame: CGRectMake(0.0, 0.0, theWidth, tabbarHeight), indicatorColor: tabIndicatorColor, indicatorHW: tabIndicatorWidthOrHeight, style: tabStyle, orientation: .Horizontal)
                } else {
                    tabView = CSTabView(frame: CGRectMake(0.0, 0.0, tabWidth, tabbarHeight), indicatorColor: tabIndicatorColor, indicatorHW: tabIndicatorWidthOrHeight, style: tabStyle, orientation: .Horizontal)
                }
                
                break
            case .Left:
                if let theHeight: CGFloat = delegate?.heightForTabAtIndex?(index) {
                    tabView = CSTabView(frame: CGRectMake(0.0, 0.0, tabbarWidth, theHeight), indicatorColor: tabIndicatorColor, indicatorHW: tabIndicatorWidthOrHeight, style: tabStyle, orientation: .VerticalLeft)
                } else {
                    tabView = CSTabView(frame: CGRectMake(0.0, 0.0, tabbarWidth, tabbarWidth), indicatorColor: tabIndicatorColor, indicatorHW: tabIndicatorWidthOrHeight, style: tabStyle, orientation: .VerticalLeft)
                }
                
                break
            case .Right:
                if let theHeight: CGFloat = delegate?.heightForTabAtIndex?(index) {
                    tabView = CSTabView(frame: CGRectMake(0.0, 0.0, tabbarWidth, theHeight), indicatorColor: tabIndicatorColor, indicatorHW: tabIndicatorWidthOrHeight, style: tabStyle, orientation: .VerticalRight)
                } else {
                    tabView = CSTabView(frame: CGRectMake(0.0, 0.0, tabbarWidth, tabbarWidth), indicatorColor: tabIndicatorColor, indicatorHW: tabIndicatorWidthOrHeight, style: tabStyle, orientation: .VerticalRight)
                }
    ...
    private func updateTabIndex(index: Int, animated: Bool) {
        if let currentTabCell = tabScrollView.cellForItemAtIndexPath(NSIndexPath(forRow: currentTabIndex, inSection: 0)) {
            (currentTabCell.contentView.subviews.first as! CSTabView).selected = false
        }
        if let newTabCell = tabScrollView.cellForItemAtIndexPath(NSIndexPath(forRow: index, inSection: 0)) {
            ...
            (newTabCell.contentView.subviews.first as! CSTabView).selected = true
        } else {
...
// MARK: - CSTabView
private class CSTabView: UIView {
    enum CSTabOrientation {
        case Horizontal
        case VerticalLeft
        case VerticalRight
    }
    ...
    var selected: Bool = false {
        didSet {
            if subviews[0] is CSTabBarItem {
                (subviews[0] as! CSTabBarItem).selected = selected
            } else {
    ...
    var orientation: CSTabOrientation = .Horizontal
    var style: CSTabStyle = .None
    
    init(frame: CGRect, indicatorColor: UIColor, indicatorHW: CGFloat, style: CSTabStyle, orientation: CSTabOrientation) {
...
// MARK: - CSPageViewController Data Source
@objc protocol CSPageViewControllerDataSource {
    /// Asks dataSource how many pages will there be.
    ///
    /// - parameter pageViewController: the CSPageViewController instance that's subject to
    ///
    /// - returns: the total number of pages
    func numberOfPagesForViewController(pageViewController: CSPageViewController) -> Int
    ...
    func tabViewForPageAtIndex(pageViewController: CSPageViewController, index: Int) -> UIView
    
    /// The content for any tab. Return a UIViewController instance and CSPageViewController will use its view to show as content.
    ///
    /// - parameter pageViewController: the CSPageViewController instance that's subject to
    ...
    func viewControllerForPageAtIndex(pageViewController: CSPageViewController, index: Int) -> UIViewController?
}

// MARK: - CSPageViewController Delegate
@objc protocol CSPageViewControllerDelegate {
